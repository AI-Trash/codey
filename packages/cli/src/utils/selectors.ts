...
export function toLocator(page: Page, target: SelectorTarget): Locator {
  ...
  if ('label' in target)
    return page.getByLabel(target.label, target.options || {})
  if ('placeholder' in target)
    return page.getByPlaceholder(target.placeholder, target.options || {})
...
export async function firstVisible(
  ...
  throw new Error(
    `None of the provided selectors became visible: ${JSON.stringify(targets)}`,
  )
}
