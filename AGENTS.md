...
- When a flow has concurrent companion work that progresses independently of the primary flow, model it as a parallel/orthogonal state-machine region (`regions`) with its own states and events. Do not add new ad-hoc companion state variables or promise-status side channels that the runner interprets outside the machine.
