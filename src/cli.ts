...
async function runExchangeCommand(
  ...
  if (subcommand === "folders") {
    ...
    console.log(
      JSON.stringify(
        { command: "exchange:folders", config: redactForOutput(config), result },
        null,
        2,
      ),
    );
  ...
  if (subcommand === "messages") {
    ...
    console.log(
      JSON.stringify(
        { command: "exchange:messages", config: redactForOutput(config), result },
        null,
        2,
      ),
    );
...
function withCommonOptions<
  TCommand extends { option(name: string, description?: string, config?: never): TCommand },
...
withCommonOptions(
  exchangeCli
    .command("verify", "Verify Exchange token, mailbox folder access, and inbox message access")
