...
function getGrantTypes(
  row: Pick<OAuthClientRow, 'clientCredentialsEnabled' | 'deviceFlowEnabled'>,
): string[] {
  ...
  if (!grantTypes.length) {
    throw new Error(
      'At least one OAuth grant must be enabled for a managed client',
    )
  }
  return grantTypes
}
...
export async function createOAuthClient(
  ...
  if (!clientCredentialsEnabled && !deviceFlowEnabled) {
    throw new Error(
      'At least one of client credentials or device flow must be enabled',
    )
  }
...
export async function updateOAuthClient(
  ...
  const deviceFlowEnabled =
    input.deviceFlowEnabled ?? existing.deviceFlowEnabled
  ...
  const effectiveSecret =
    rotatedSecret || decryptClientSecret(row.clientSecretCiphertext)
