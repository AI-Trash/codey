...
- WebSocket delivery of verification codes and admin notifications to connected CLI clients
...
- `exchange` â€?preserve the original Exchange / Graph polling behavior
- `app` â€?reserve email aliases from the app and receive verification codes via app WebSocket / Cloudflare ingest
...
The CLI keeps a WebSocket connection to `/api/realtime/ws`, waits for tasks dispatched from `/admin/cli`, and reports its current flow back to the web app so the admin page can show what is running.
...
`daemon start` still works for non-interactive terminals and keeps the same worker loop, but the prompt CLI is now the preferred operator-facing mode.
