...
Verification mailboxes are now managed in the admin console at `/admin/mailboxes` and stored in Postgres. App-backed CLI registrations now randomly pick one mailbox route marked for new registrations for each reserved address instead of binding a domain to the OAuth client. Cloudflare mailbox routes use a domain routed to the existing email worker. Personal Outlook mailbox routes use the configured Outlook address directly and are skipped after they already have a reservation, so each personal mailbox is assigned without a plus-address tag. Reserved Cloudflare mailboxes use the generated memorable mailbox name by default; a prefix such as `codey` is only prepended when that specific Cloudflare mailbox route has an explicit mailbox prefix configured.
...
Codey Desktop is the Tauri-based local task manager for Codey automation
flows. The desktop app owns queueing, bounded local concurrency, structured
status events, per-task logs, cancellation, persisted desktop settings, and an
in-app updater entry point.

The automation host lives under `packages/desktop/src-host` and calls the flow
...
The updater UI is wired through Tauri's updater plugin. Release builds still
need a real updater endpoint and signing public key in
`packages/desktop/src-tauri/tauri.conf.json` before automatic updates can be
distributed.
...
pnpm codey chatgpt-register-hosted-checkouts
