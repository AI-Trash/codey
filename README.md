...
`chatgpt-register-hosted-checkouts` creates a new ChatGPT account, generates hosted GoPay checkout links for the built-in country list, and opens one checkout page at a time. Close the active checkout tab/window to advance to the next country; after the last page closes, the flow writes the generated link list under `artifacts/`.
