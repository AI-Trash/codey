...
async function oidcHandler(
  ...
  try {
    ...
    const interactionMatch = pathname.match(
      /^\/oidc\/interaction\/([^/]+)(?:\/(confirm|abort|done))?$/,
    )
    ...
    if (interactionMatch) {
      ...
      if (!action && req.method === 'GET') {
        await sendResponse(
          res,
          await loadInteractionPage(request, uid, req, res),
        )
        return
      }
      if (action === 'confirm' && req.method === 'POST') {
        await sendResponse(
          res,
          await completeInteraction(request, uid, false, req, res),
        )
        return
      }
      if (action === 'abort' && req.method === 'POST') {
        await sendResponse(
          res,
          await completeInteraction(request, uid, true, req, res),
        )
        return
      }
